...
- Tokens JWT contendo informações do usuário e role.
    Obs.: A secret para assinatura dos JWTs foi gerada randomicamente com os algoritmos da biblioteca JJWT.
...
- **Criação:** admin pode criar novos usuários com nome, e-mail e role.
    Obs.:  Senhas atualmente não podem ser alteradas e são geradas randomicamente como UUID, a senha é exibida no console interno na hora da criação.
...
2.Builde e execute com Spring Boot
3.Acesse no navegador "http://localhost:8080/page/login" (ou configure a porta à sua preferência)
  Está disponível no package test um algoritmo de inicialização do primeiro usuário (Admin) no banco, para que possa ser realizado os testes.
